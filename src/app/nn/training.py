...
def get_stats(model, data_in, data_out):
    output = model.forward(data_in, data_in.shape[0])
    return np.mean(categorial_cross_entropy(output, data_out)), np.sum(output * data_out) / data_in.shape[0] * 100

def batched(model, data_in, data_out, epochs, recompile = True, learning_rate = 0.01, batch_size = 256,
