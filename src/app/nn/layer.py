...
class TrainableLayer(StaticLayer):
    trainable = True
    id = 0
...
class Convolution(TrainableLayer):
    ...
    def apply_col2im(view, delta, batch_size, output_size, input_channels):
        for b in nb.prange(batch_size):
            for c in nb.prange(input_channels):
                for i in nb.prange(output_size):
                    for j in nb.prange(output_size):
                        view[b, c, i, j] += delta[b, c, i, j]
    ...
    def col2im(self, gradient, batch_size):
        ...
        self.apply_col2im(view, delta, batch_size, self.output_size, self.input_channels)
        return gradient
...
class MaxPool(StaticLayer):
    ...
    @staticmethod
    @nb.njit(parallel = True)
    def apply_gradient(delta, gradient, kernels, batch_size, input_size, output_size, filter_size, channels):
        for i in nb.prange(output_size):
            for j in nb.prange(output_size):
                indices = (i * filter_size + kernels[:, :, i, j] // filter_size) * input_size + (j * filter_size + kernels[:, :, i, j] % filter_size)
                for b in nb.prange(batch_size):
                    for c in nb.prange(channels):
                        delta[b, c, indices[b, c]] = gradient[b, c, i, j]

    def backpropagate(self, gradient, delta_multiplier, batch_size):
        delta = np.zeros((batch_size, self.channels, self.input_size ** 2))
        self.apply_gradient(delta, gradient, self.kernels, batch_size, self.input_size, self.output_size, self.filter_size, self.channels)
